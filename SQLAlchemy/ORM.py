...
class SyncORM:
    ...
    @staticmethod
    def insert_tables_client(*_client_name):  # функция доабвления данных в таблицу (если данные много но по 1 на 1 объект)
    ...
    def select_client(temp_id):  # Функция выбора клиента из списка (можно выбрать одного, моэжно всех)
        ...
        with session_factory() as session:
            if temp_id == 0:
                ...
                clients = result.scalars().all()  # отображаем выбранных клиентво (скаляр для отсеивания ненужных скобок)
            else:
    ...
    def update_client(clients_id, new_name):      # обновляем данные в таблице клиентво
        ...
        with session_factory() as session:
            ...
            session.commit()
    @staticmethod
    def avg_pets_age(temp_part_breed, temp_age):                  # фукнция выборки из таблицы питомцев среднего возраста для метисов и британцев
