...
new_human = []  # Наш список для храениня данных по человекам (в рамках текущего функционала- бесполезен),
# но имхо плохо когда данные не хранятся
...
def start(message, res=False):
    bot.send_message(message.chat.id, 'Вас приветствует дикий ИскИн. Если Вы будете вести себя хорошо я помогу Вам '
                                      'подобрать машину исходя из нужной Вам марки и имеющихся у Вас денег! Если '
                                      'Вы готовы отправьте слово "готов" в чат. )')


# Получение сообщений от юзера
@bot.message_handler(content_types=['text'])
def get_text_messages(message):
    if message.text == "готов" or message.text == 'Неверно' or message.text == 'ujnjd':
        bot.send_message(message.from_user.id, "Для формирования списка ответьте на несколько вопросов!")
...
def process_money_step(message):  # Функция пошагового запроса для получения данных (деньги)
    try:
        ...
        money = message.text
        if not money.isdigit():  # Проверка на то что бы денежки были цифрой
            message = bot.reply_to(message, 'Значение денег - это цифра')
